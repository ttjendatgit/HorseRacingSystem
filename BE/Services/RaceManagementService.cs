...
public class RaceManagementService : IRaceManagementService
{
    ...
    public async Task<ServiceResult<bool>> StartRaceAsync(Guid raceId)
    {
        try
        {
            ...
            if (race.Tournament != null && race.Tournament.Status == TournamentStatus.Published)
            {
                race.Tournament.Status = TournamentStatus.Ongoing;
                race.Tournament.StartedAt = DateTime.UtcNow;
                race.Tournament.UpdatedAt = DateTime.UtcNow;
                await _unitOfWork.SaveChangesAsync();
            }
            else if (race.Tournament?.Status != TournamentStatus.Ongoing)
            {
