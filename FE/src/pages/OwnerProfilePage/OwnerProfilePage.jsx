...
export default function OwnerProfilePage() {
  ...
  useEffect(() => {
    getProfile()
      .then((d) => {
        const p = d?.data ?? d;
        setProfile(p);
        setInfo({ fullName: p.fullName ?? p.FullName ?? "", phoneNumber: p.phoneNumber ?? p.PhoneNumber ?? "" });
      })
      .catch(() => { /* empty */ })
  ...
  return (
    <ProfileLayout profile={profile} roleLabel="Chủ ngựa" tabs={OWNER_TABS} activeTab={activeTab} setActiveTab={setActiveTab}>
      ...
      {activeTab === "info" && (
        <section className="sp-section">
          ...
          {!editMode && (
            <div style={grid2}>
              <Detail label="Vai trò" value="Chủ ngựa" />
              <Detail label="Mã" value={profile.code ?? profile.Code ?? "-"} />
