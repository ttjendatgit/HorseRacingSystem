...
function RefereeHealthCheckPage() {
  ...
  const { total, passed, failed, recheck } = useMemo(() => {
    ...
    const p = healthChecks.filter((c) => (c.status || c.Status) === "Passed").length;
    const f = healthChecks.filter((c) => (c.status || c.Status) === "Failed").length;
    const r = healthChecks.filter((c) => (c.status || c.Status) === "RequiresRecheck").length;
                      ...
                      {availableEntries.map((entry) => (
                        <option key={entry.horseId || entry.HorseId} value={entry.horseId || entry.HorseId}>
                          ...
                          {(entry.jockeyName || entry.JockeyName)
                            ? ` (Nài: ${entry.jockeyName || entry.JockeyName})`
                            : ""}
                        </option>
                      ))}
                    </select>
              ...
              {loading ? (
                ...
                <div className="rh-list">
                  {healthChecks.map((check) => {
                    const checkId = check.id || check.Id;
                    const cStatus = check.status || check.Status;
                    const isApproved = check.approvedToRace || check.ApprovedToRace;
                    return (
                      <div key={checkId} className="rh-item">
                        <div className="rh-item__top">
                          <strong className="rh-item__name">
                            {check.horseName || check.HorseName || "Ngựa #" + (check.horseId || check.HorseId)}
                          </strong>
                          <span
                            className={`rh-badge rh-badge--${cStatus?.toLowerCase()}`}
                          >
                            {STATUS_MAP[cStatus] || cStatus}
                          </span>
                        </div>

                        {(check.jockeyName || check.JockeyName) && (
                          <div className="rh-item__meta">
                            Nài: {check.jockeyName || check.JockeyName}
                          </div>
                        )}

                        {(check.notes || check.Notes || check.observations || check.Observations) && (
                          <p className="rh-item__notes">{check.notes || check.Notes || check.observations || check.Observations}</p>
                        )}

                        <div className="rh-item__bottom">
                          <span className="rh-item__time">
                            {(check.createdAt || check.CreatedAt)
                              ? new Date(check.createdAt || check.CreatedAt).toLocaleString("vi-VN")
                              : ""}
                          </span>
                              ...
                              cStatus === "Passed" && (
                                <>
                                  <button
                                    ...
                                    onClick={() => handleApprove(checkId)}
                                  >
                                    Phê duyệt
                                  </button>
                                  <button
                                    className="rh-btn rh-btn--sm rh-btn--reject"
                                    onClick={() => handleReject(checkId)}
                                  >
                                    Từ chối
                                  </button>
                                </>
