          ...
          <div style={{marginBottom:16}}>
            ...
            {selectedTrack && (
              <div style={{marginTop:8,display:"grid",gridTemplateColumns:"repeat(3, 1fr)",gap:8,padding:"8px 12px",borderRadius:8,background:"var(--hr-surface-2)",border:"1px solid var(--hr-border-soft)"}}>
                ...
                <div><span style={{fontSize:10,color:"var(--hr-muted)",display:"block"}}>Chiều dài đường đua</span><strong style={{fontSize:13,color:"var(--hr-paper)"}}>{(selectedTrack.length ?? selectedTrack.Length) ? `${selectedTrack.length ?? selectedTrack.Length}m` : "Chưa có"}</strong></div>
            ...
            {showTrackForm && (
              <div style={{marginTop:8,padding:12,borderRadius:8,border:"1px solid var(--hr-border-soft)",background:"var(--hr-surface-2)"}}>
                <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:8}}>
                  ...
                  <input className="hr-field" type="number" min="1" value={newTrack.length} onChange={(e) => setNewTrack({...newTrack, length: e.target.value})} placeholder="Chiều dài đường đua (m)" style={{padding:"8px 12px",borderRadius:8,border:"1px solid var(--hr-border-soft)",background:"var(--hr-surface)",color:"var(--hr-text)",fontSize:13}} />
                ...
                <p style={{margin:"0 0 8px",fontSize:11,color:"var(--hr-muted)"}}>"Chiều dài đường đua" là thông số vật lý cố định của đường đua — khác với "Khoảng cách cuộc đua" bên dưới (quãng đường thi đấu của riêng Cuộc đua này).</p>
                <div style={{display:"flex",gap:8}}>
